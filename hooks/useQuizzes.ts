...
export function useQuizzes({
  ...
  const fetchMoreQuizzes = useCallback(
    async (pageNum: number) => {
      ...
      try {
        ...
        setQuizzes((prev) => {
          const newQuizzes = [...prev, ...(data as Quiz[])];
          return Array.from(new Map(newQuizzes.map((q) => [q.quiz_id, q])).values());
        });
        setHasMore(data.length === quizzesPerPage);
      } catch (error: unknown) {
    ...
    [userId, publicOnly, searchQuery, quizzesPerPage]
  );
  ...
  useEffect(() => {
    ...
    setQuizzes([]); // Reset quizzes on userId, publicOnly, or searchQuery change
    setPage(0);
    fetchMoreQuizzes(0);
  }, [userId, publicOnly, searchQuery, fetchMoreQuizzes]);
