...
import { InfoLdg, InfoList } from "@/types";
      ...
      <MaxWidthWrapper className="grid gap-10 px-2.5 lg:grid-cols-2 lg:items-center lg:px-7">
        <div className={cn(reverse ? "lg:order-2" : "lg:order-1")}>
          ...
          <dl className="mt-6 space-y-4 leading-7">
            {data.list.map((item: InfoList, index: number) => {
              const iconKey = item.icon as keyof typeof Icons; // Ensure iconKey is a valid key of Icons
              const Icon = Icons[iconKey] || Icons.arrowRight; // Fallback to arrowRight if icon is invalid
              return (
